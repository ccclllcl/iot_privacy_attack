"""针对 canonical artifacts 运行单组合 Dashboard demo 任务。"""
...
def _target_dir(dataset: str, seed: int, model: str, job: str, method: str | None) -> Path:
    ...
    if method is None:
        raise ValueError("防御任务必须指定 --method")
...
def build_demo_config(
    ...
    if not isinstance(raw, dict):
        raise ValueError(f"配置文件格式无效：{rel_path(cfg_path)}")
    ...
    *,
    ...
    _write_json(
        target / "source_manifest.json",
        {
            ...
            "note": "由 Dashboard 单组合 demo runner 生成；未执行数据导入，也未运行完整矩阵。",
        },
    )
...
def execute_dashboard_job(
    ...
    if not processed.exists() or not seq_path.exists() or (model == "mlp" and not mlp_path.exists()):
        raise FileNotFoundError(f"缺少 processed 数据：{dataset}/seed_{seed}，路径：{rel_path(processed)}")
    ...
    if job.startswith("defense") and method:
        ...
        if not defended.exists():
            raise FileNotFoundError(f"缺少 defended 数据：{rel_path(defended)}")
    ...
    if _is_complete(target, job, model_path) and not overwrite:
        raise FileExistsError(f"拒绝覆盖已有产物：{rel_path(target)}。如需运行，请启用 overwrite。")
    ...
    if job == "train_baseline":
        ...
        if not baseline_model.exists():
            raise FileNotFoundError(f"缺少 baseline 模型：{rel_path(baseline_model)}")
...
def write_run_history(record: dict[str, Any]) -> None:
    """追加写入 `outputs/ui/run_history.jsonl`。"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="运行一个 canonical Dashboard 单组合训练/评估任务。")
